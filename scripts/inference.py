...
horizon = 20 # number of tokens generated in each sample
...
ckpt_path = "runs/gpt2_20250223_105627/ckpt.pt"
...
start_ids = data[:-horizon]
print("True continuation:")
print(data[-horizon:].to(torch.uint8).view(torch.float32))
print('---------------')
...
with torch.no_grad():
    with ctx:
        for k in range(num_samples):
            y = model.generate(x, horizon, temperature=temperature, top_k=top_k)
            candle = y[0, -horizon:].to(torch.uint8).view(torch.float32)
            print(candle)
